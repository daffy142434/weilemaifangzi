<template lang="html">
  <div>
    ...
    <div v-if="$store.state.powers.includes('member:main_add')" class="tools-bar">
      <el-button v-if="$store.state.powers.includes('member:main_add')" type="success" icon="el-icon-plus" size="small" @click="dialogVisible = true;dialogTitle='新增用户'">新增用户</el-button>
    </div>
    <div v-if="$store.state.powers.includes('member:main_list')">
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div>
              <el-button v-if="$store.state.powers.includes('member:main_modify')" type="text" size="small" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
              <el-button v-if="$store.state.powers.includes('member:main_reset_pwd')" type="text" size="small" @click="handleResetPwd(scope.$index, scope.row)">重置密码</el-button>
              <!-- <el-button v-if="scope.row.status==1&&$store.state.powers.includes('member:main_active')" type="text" size="small" @click="handleChangeStatus(scope.$index, scope.row)">停用</el-button>
              <el-button v-if="scope.row.status==0&&$store.state.powers.includes('member:main_active')" type="text" size="small" @click="handleChangeStatus(scope.$index, scope.row)">启用</el-button> -->
            </div>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    handleStatus(row){
      ...
      ajax.post('/members/'+row.id+'/active', {activeStatus})
        .then((response) => {
          if(response.data.code == 200) {
            this.$message({
              showClose: true,
    ...
    onSearch({pageNumber = 1} = {}) {
      ...
      ajax.post('/members/list', Object.assign({pageNumber, pageSize}, this.searchData))
      .then((response) => {
    ...
    handleResetPwd(index, row) {
      this.$confirm('确认重置该用户的登录密码？', '提示', {
        ...
        ajax.post('/members/'+row.id+'/reset/pwd')
        .then((response) => {
          if(response.data.code == 200) {
            this.$message({
              showClose: true,
    ...
    onDialogSubmit() {
      ...
      let url = dataForm.id ? ('/members/'+this.dataForm.id+'/modify') : '/members/add';
        ...
        if (valid) {
          ...
          .then((response) => {
            if(response.data.code == 200) {
              ...
              this.$message({
                duration: dataForm.id ? 3*1000 : 30*1000,
                showClose: true,
